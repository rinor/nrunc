...
URUNC_SRC      := $(wildcard $(CURDIR)/cmd/$(URUNC_BIN_NAME)/*.go)
...
$(URUNC_BIN)_static_%: $(URUNC_SRC) | prepare
	$(GO_FLAGS) GOARCH=$* $(GO) build --trimpath \
		...
		-o $(URUNC_BIN)_static_$* $(CURDIR)/cmd/$(URUNC_BIN_NAME)

$(URUNC_BIN)_dynamic_%: $(URUNC_SRC) | prepare
	$(GO_FLAGS) GOARCH=$* $(GO) build --trimpath \
		-ldflags "$(LDFLAGS_COMMON) $(LDFLAGS_OPT)" \
		-o $(URUNC_BIN)_dynamic_$* $(CURDIR)/cmd/$(URUNC_BIN_NAME)

$(SHIM_BIN)_%: $(SHIM_SRC) | prepare
	@sed -i 's/DefaultCommand = "runc"/DefaultCommand = "$(URUNC_BIN_NAME)"/g' \
...
install: $(SHIM_BIN)_$(ARCH) $(INSTALL_DEPS)
	install -D -m0755 $(word 2,$^) $(PREFIX)/$(URUNC_BIN_NAME)
	install -D -m0755 $< $(PREFIX)/containerd-shim-$(URUNC_BIN_NAME)-v2

## uninstall Remove urunc and shim from PREFIX
.PHONY: uninstall
uninstall:
	rm -f $(PREFIX)/$(URUNC_BIN_NAME)
	rm -f $(PREFIX)/containerd-shim-$(URUNC_BIN_NAME)-v2
